...
- Use `bcryptjs` for local password hashing in this project. Use only its asynchronous hash/compare APIs, reject inputs for which `bcrypt.truncates(password)` is true, and configure a benchmarked work factor of at least 10 through validated environment configuration.
